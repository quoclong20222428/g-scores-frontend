      ...
      <div className="mb-4 sm:mb-6">
        ...
        <div className="max-w-md">
          ...
          <select
            ...
            className="w-full px-3 sm:px-4 py-2 border-2 border-gray-300 rounded-lg text-sm sm:text-base text-gray-700 font-medium hover:border-blue-400 transition focus:outline-none focus:border-blue-600"
          >
            {blocks.map((block) => (
        ...
        <div className="overflow-x-auto -mx-4 sm:mx-0">
          <table className="w-full text-sm sm:text-base">
            <thead className="bg-gray-100 border-b-2 border-gray-300">
              <tr>
                <th className="px-2 sm:px-4 py-2 sm:py-3 text-left text-xs sm:text-sm font-bold text-gray-700">
                  Rank
                </th>
                <th className="px-2 sm:px-4 py-2 sm:py-3 text-left text-xs sm:text-sm font-bold text-gray-700">
                  Reg. No.
                ...
                {blocks
                  ...
                  ?.subjects.map((subject) => (
                    <th
                      key={subject.key}
                      className="px-1 sm:px-4 py-2 sm:py-3 text-center text-xs sm:text-sm font-bold text-gray-700"
                    >
                      <span className="hidden sm:inline">{subject.name}</span>
                      <span className="sm:hidden">{subject.name.split(' ')[0]}</span>
                    </th>
                  ))}
                <th className="px-2 sm:px-4 py-2 sm:py-3 text-right text-xs sm:text-sm font-bold text-gray-700">
                  Total
                </th>
              </tr>
            </thead>
            <tbody>
              {topStudents.map((student, index) => {
                ...
                return (
                  <tr
                    key={student.sbd}
                    className={`border-b text-xs sm:text-base ${
                  ...
                  >
                    ...
                    <td className="px-2 sm:px-4 py-2 sm:py-4 text-gray-900 font-medium">
                      {student.sbd}
                    </td>
                    {currentBlock?.subjects.map((subject) => (
                      <td
                        key={subject.key}
                        className="px-1 sm:px-4 py-2 sm:py-4 text-center text-gray-700"
                      >
                    ...
                    <td className="px-2 sm:px-4 py-2 sm:py-4 text-right font-bold text-gray-900">
